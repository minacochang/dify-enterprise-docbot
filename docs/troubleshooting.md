...
**症状**: `./docbot.py "検索語"` で「0件。検索対象フィールド/言語を確認」と表示される。

**確認**:
1. **DB の有無**: `index.db` が存在するか。無ければ `python ingest.py` を実行
...
**症状**: `./docbot.py helm "..."` で「helm template 失敗」と表示される。
...
**症状**: `./docbot.py compose "..."` で YAML 取得に失敗する。
