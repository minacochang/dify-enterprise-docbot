...
```
┌─────────────┐     POST /search      ┌──────────────┐     FTS5      ┌─────────────┐
│  docbot.py  │ ──────────────────▶  │  server.py   │ ────────────▶ │  index.db   │
│  (CLI)      │  http://127.0.0.1:8000│  (FastAPI)   │               │  (SQLite)   │
└─────────────┘                       └──────────────┘               └─────────────┘
       │                                      │
...
- **ingest.py**: クロールして `index.db` に格納
- **storage.py**: SQLite FTS5 で全文検索、ja-jp は N-gram + 再スコア
- **extract.py**: HTML から title/lead/headings/body 抽出
